...
static int dump_sit_link(NetDeviceEntry *nde, struct cr_imgset *fds, struct nlattr **data)
{
	...
	/* sit0 is special, users can't actually modify it, but it holds all
	 * the state for other sit tunnels in structures not visible to
	 * userspace. In any case, we don't need to dump it.
	 */
	if (!strcmp(nde->name, "sit0"))
		return 0;
