...
#define MS_LOCK_FLAGS (MS_NOATIME | MS_NODIRATIME | MS_RELATIME | MS_NOSUID | MS_NODEV | MS_NOEXEC | MS_RDONLY)
...
static int do_bind_mount(struct mount_info *mi)
{
	...
	unsigned long mflags, lockflags;
	int exit_code = -1;
	bool shared = false;
	bool master = false;
	bool private = false;
...
do_bind:
	...
	/* We can't change various flags after they've been set as a user, so
	 * let's try to set them the first time.
	 */
	lockflags = mi->flags & MS_LOCK_FLAGS;

	if (mount(root, mi->mountpoint, NULL, MS_BIND | lockflags, NULL) < 0) {
		pr_perror("Can't mount at %s", mi->mountpoint);
		goto err;
	}

	mflags = mi->flags & (~MS_PROPAGATE | ~MS_LOCK_FLAGS);
	if (!mi->bind || mflags != (mi->bind->flags & (~MS_PROPAGATE | ~MS_LOCK_FLAGS)))
