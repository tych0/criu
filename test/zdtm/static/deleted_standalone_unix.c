...
static int bind_and_maybe_delete(struct sockaddr_un *addr, bool delete)
{
	int sk;
	...
	if (delete && unlink(addr->sun_path) < 0) {
		fail("unlink");
		close(sk);
		return -1;
	}
...
int main(int argc, char **argv)
{
	...
	sk1 = bind_and_maybe_delete(&addr, false);
	if (sk1 < 0)
		goto out;

	test_msg("bound first\n");
	...
	sk2 = bind_and_maybe_delete(&addr, true);
	if (sk2 < 0)
		goto out;
